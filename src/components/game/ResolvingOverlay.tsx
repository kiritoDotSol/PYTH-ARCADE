...
export const ResolvingOverlay: React.FC<ResolvingOverlayProps> = ({ resolvingIndex, tempScore, tempCombo }) => {
  ...
  return (
    <motion.div
      ...
      className="w-full max-w-xl bg-background border border-border p-6 sm:p-8 rounded-3xl shadow-2xl relative overflow-hidden"
    >
      <div className="absolute top-0 left-0 w-full h-1 bg-brand-lime" />
      
      <div className="text-center mb-6">
        <span className="text-[10px] font-mono text-foreground/40 uppercase tracking-[0.4em] mb-2 block animate-pulse">Market Settlement</span>
      ...
      <div className="space-y-3 max-h-[40vh] overflow-y-auto pr-2 custom-scrollbar">
        {predictions.map((p, index) => {
          ...
          return (
            <motion.div 
              ...
              className={`flex items-center justify-between p-4 rounded-xl border transition-all ${isRevealed ? (isCorrect ? 'bg-brand-lime/10 border-brand-lime/30' : 'bg-rose-500/10 border-rose-500/30') : 'bg-white/5 border-border'}`}
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-bold text-foreground text-base">{p.symbol}</p>
                  <p className="text-[10px] font-mono text-foreground/40">Entry: ${p.spawnPrice.toFixed(2)}</p>
                </div>
              </div>
        ...
        {predictions.length === 0 && (
          <div className="text-center py-12 text-foreground/30 font-mono text-sm uppercase tracking-widest border border-dashed border-border rounded-2xl">
      ...
      <div className="mt-8 pt-6 border-t border-border flex justify-between items-end">
        <div className="flex flex-col gap-1">
          <p className="text-[10px] font-mono text-foreground/40 uppercase tracking-widest">Projected P&L</p>
          <p className="text-3xl font-black text-foreground italic tracking-tighter">${tempScore.toLocaleString()}</p>
        </div>
        <div className="flex flex-col items-end gap-1">
          <p className="text-[10px] font-mono text-foreground/40 uppercase tracking-widest">Current Streak</p>
